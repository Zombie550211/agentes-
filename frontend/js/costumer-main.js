(function () {
  ...
  async function fetchLeads(month) {
    ...
    // month=undefined → carga inicial → backend aplica mes actual (rápido)
    // month=''       → usuario eligió "Todos" → noAutoMonth=1 para cargar todos
    // month='YYYY-MM' → mes específico → fechaInicio/fechaFin
    let dateParams = '';
    if (month) {
      ...
      // Explícitamente "Todos": deshabilitar filtro de mes actual del backend
      dateParams = '&noAutoMonth=1';
    }
  ...
  function applyFiltersDebounced(){clearTimeout(applyFiltersTimer);applyFiltersTimer=setTimeout(applyFilters,350);}
